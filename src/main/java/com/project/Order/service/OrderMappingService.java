...
public class OrderMappingService {
    ...
    public OrderMapping mapOrder(OrderMapping orderMapping) {
        ...
        Items reqI= itemsRepository.findById(orderMapping.getItemId()).get();
        if(reqI.getStock()<=0){
            System.out.println("ITEM Out of Stock");
            return null;
        }else if (orderMapping.getQuantity() >= 5) {
            ...
            return null;
        } else if (reqI.getStock()<orderMapping.getQuantity()){

            System.out.println("You are ordering "+orderMapping.getQuantity() +".But current stock is "+reqI.getStock()+".So order in limit");
            return null;
        }
           else {
...
//            cart.setTotal(total);
                ...
                reqI.setStock((reqI.getStock())-(orderMapping.getQuantity()));
                itemsRepository.save(reqI);
                return cart1;
            }
        }
